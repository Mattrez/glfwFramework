...
void ShaderAtlas::addShader(ShaderId ID, const char* vertexShaderPath, const char* fragmentShaderPath)
{
	Shaders.emplace(std::make_pair(ID, std::make_shared<Shader> (vertexShaderPath, fragmentShaderPath)));
}
